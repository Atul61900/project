...
export default function SearchDropdown({ results, loading, query, onResultClick }) {
  ...
  return (
    <div className="absolute top-full right-0 mt-3 w-[22rem] sm:w-[26rem] glass-premium rounded-3xl overflow-hidden z-[60] origin-top-right animate-fadeUp shadow-2xl">
      <div className="p-3 border-b border-slate-200/50 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01]">
        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-2">Top Matches</p>
      </div>
      ...
      <Link
        ...
        onClick={onResultClick}
        className="block p-4 text-center text-xs font-black text-teal-600 bg-teal-50/50 hover:bg-teal-50 dark:text-teal-400 dark:bg-teal-500/5 dark:hover:bg-teal-500/10 border-t border-slate-200/50 dark:border-white/5 transition-all tracking-widest"
      >
        VIEW ALL RESULTS
      </Link>
    </div>
