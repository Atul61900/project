...
export default function SpotlightSlider({ items }) {
  ...
  return (
    <div 
      className="relative w-full h-[55vh] sm:h-[60vh] md:h-[65vh] bg-transparent overflow-hidden group select-none transition-colors duration-500"
          ...
          >
            ...
            >
              <div className="max-w-4xl pointer-events-none drop-shadow-2xl">
                {/* Spotlight Badge */}
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-teal-500/20 to-cyan-500/20 text-teal-300 border border-teal-500/30 rounded-full font-black text-[10px] uppercase tracking-widest mb-5 shadow-lg backdrop-blur-md">
                   <span className="w-2 h-2 rounded-full bg-teal-400 pulse-active" />
                   Spotlight #{index + 1}
                </div>

                {/* Title — Optimized for all poster types */}
                <h1 className="font-display text-4xl sm:text-5xl md:text-7xl font-black text-white mb-4 leading-[0.95] tracking-tighter drop-shadow-[0_4px_16px_rgba(0,0,0,0.6)] max-w-3xl">
                  {series?.title}
                </h1>
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm font-bold text-slate-200 mb-6 drop-shadow-md">
                  ...
                  {series?.genres?.slice(0,2).map(g => (
                     <span key={g} className="text-white/80 uppercase tracking-wider text-[11px]">{g}</span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4 pointer-events-auto">
                   <button 
                     ...
                     className="px-8 py-3.5 bg-gradient-to-r from-teal-500 to-teal-400 text-charcoal-950 font-black rounded-xl btn-glow flex items-center gap-2.5 transform"
                   >
                   ...
                   <button 
                     ...
                     className="px-8 py-3.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl transition-all duration-300 backdrop-blur-md border border-white/20 flex items-center gap-2.5 hover:scale-[1.03] active:scale-[0.98] shadow-lg"
                   >
