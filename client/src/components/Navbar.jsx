...
const link = ({ isActive }) =>
  `relative px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider font-extrabold transition-all duration-300 ${
    isActive
      ? 'text-teal-500 bg-teal-500/10 dark:text-ice-400 dark:bg-teal-500/15 shadow-[0_0_15px_-3px_rgba(20,184,166,0.3)] border border-teal-500/20'
      : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100/50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/[0.04] border border-transparent'
  }`;
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/70 backdrop-blur-2xl shadow-[0_1px_15px_rgba(15,23,42,0.02)] dark:border-white/[0.06] dark:bg-charcoal-950/70 dark:shadow-[0_8px_32px_-8px_rgba(0,0,0,0.6)] transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[4.5rem] flex items-center justify-between gap-4">
        <div className="flex items-center gap-8 flex-1">
          <Link
            to="/"
            className="font-display font-black text-xl tracking-tight text-slate-900 dark:text-white flex items-center gap-2.5 group shrink-0"
          >
            <span className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-400 text-white shadow-[0_0_15px_rgba(20,184,166,0.4)] group-hover:scale-105 transition-all duration-300">
              <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24" className="translate-x-0.5"><path d="M8 5v14l11-7z"/></svg>
            </span>
            <span className="text-gradient-teal">ClickWatch</span>
          </Link>

          {!isSearchOpen && (
            <nav className="hidden md:flex items-center gap-1.5 p-1 rounded-2xl bg-slate-100/40 border border-slate-200/50 dark:bg-white/[0.02] dark:border-white/[0.04] backdrop-blur-md">
