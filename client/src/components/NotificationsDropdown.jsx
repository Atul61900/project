...
export default function NotificationsDropdown() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-3 w-80 sm:w-96 rounded-3xl glass-premium overflow-hidden z-50 animate-in fade-in slide-in-from-top-4 duration-200 shadow-2xl">
          <div className="flex items-center justify-between p-4 border-b border-slate-200/50 dark:border-white/5 shrink-0 bg-slate-50/50 dark:bg-white/[0.01]">
            <h3 className="font-bold text-slate-800 dark:text-white text-sm uppercase tracking-wide">Notifications</h3>
            {hasUnread && (
              <button 
                onClick={markAllRead}
                className="text-xs text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 font-extrabold px-2.5 py-1 rounded-lg hover:bg-teal-500/10 transition-colors"
              >
                Mark all read
              </button>
            )}
